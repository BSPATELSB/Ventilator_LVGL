...
#define COLOR_SIDEBAR_ACTIVE    (theme_get_palette()->btn_active)
...
static void clock_timer_cb(lv_timer_t * timer)
{
    ...
    if (arc_battery && lbl_battery_val) {
        battery_info_t bat_info;
        battery_detect_read(&bat_info);
        lv_arc_set_value(arc_battery, bat_info.capacity);
        lv_label_set_text_fmt(lbl_battery_val, "%d%%\n#7097ba %s#", bat_info.capacity, bat_info.is_charging ? "Charging" : "Good");
        lv_label_set_recolor(lbl_battery_val, true);
    }
}
